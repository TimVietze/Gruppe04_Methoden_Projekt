...
================================================================================
ML Prototype & Streamlit Application  (Phase 2–3)
================================================================================

Goal
----
Forecast one-year-ahead mid-tier single-family residential (SFR) value growth
across 855 US metropolitan areas (CBSAs), using Zillow ZHVI SFR mid-tier as a
proxy for regional owner-occupied housing market attractiveness.

The model supports investment decision-making by ranking CBSAs based on
predicted housing value growth.  It does not predict general property values,
real estate returns, or overall market conditions.


Target variable
---------------
    target_growth_1y = (zhvi_sfr_mid_dec_{t+1} - zhvi_sfr_mid_dec_t)
                       / zhvi_sfr_mid_dec_t

Derived from the December ZHVI SFR mid-tier value per CBSA per year (t).


Modeling-ready tables
---------------------
    Methoden Data/Modeling Data/Modeling_Table.csv         (stage 5, monthly)
    Methoden Data/Modeling Data/Annual_Modeling_Table.csv  (stage 6, annual)

Stage 6 (6_AnnualModelingTable.py) collapses the monthly table to annual grain,
builds the target column and lag features, and assigns a time-based split label:

    split == "train"    YEAR 2010-2021   (9,791 CBSA-year rows)
    split == "test"     YEAR 2022-2024   (2,565 CBSA-year rows)
    split == "predict"  YEAR 2025        (  855 CBSA rows, target is null)

No randomised shuffle is used.  The split is a strict forward cutoff on YEAR,
ensuring that no future data leaks into training.


How to run
----------
Run stages 1-5 first (see Pipeline section above), then:

    python3 "Data Cleaning Scripts/6_AnnualModelingTable.py"
    python3 modeling/01_ml_prototype.py
    streamlit run app/streamlit_app.py

Dependencies: requirements.txt (streamlit, pandas, numpy, scikit-learn, joblib).


Output files
------------
    app/data/metrics.csv              model comparison table (MAE, RMSE, R2, directional accuracy)
    app/data/predictions_test.csv     actual vs predicted growth for test years 2022-2024
    app/data/investment_ranking.csv   2026 growth forecasts and recommendation labels for all 855 CBSAs
    app/models/best_model.pkl         serialised best model pipeline (scikit-learn)


Main findings
-------------
Best model: Ridge regression, Feature set A (housing history only).
    Features used:  current ZHVI, prior-year ZHVI, prior-year growth rate.
    Selection:      highest directional accuracy on test set (79.9%),
                    tie-break lowest MAE (4.15 percentage points).

Weather / disaster indicators (Feature set C-lite):
    Adding NOAA storm damage and FEMA major disaster flags did NOT improve
    performance over housing history alone at CBSA-annual aggregation.
    This is a valid scientific finding.  These features are retained in the
    app as association-based risk indicators only; no causal claim is made.

Ranking backtest (2022-2024):
    Top-decile predicted metros outperformed bottom-decile metros by ~4 pp on
    average across all test years.  Top 10% hit rate (positive actual growth):
    82.5%.  The model should be interpreted as a directional ranking / screening
    tool, not a precise price-growth forecast.

Test-set R2 context:
    R2 is negative on the test set.  This is expected: training-period mean
    growth (~+5.5%) is substantially higher than the post-COVID test-period mean
    (~+2.5%), causing a distribution shift that penalises R2.  Directional
    accuracy (~80%) is the more informative metric in this context.


Limitations
-----------
- No economic variables.  Unemployment, income, mortgage rates, and inflation
  are not included.  Their absence is the largest known gap in the model.
- No confidence intervals.  The model produces point predictions only.
- Geographic grain: CBSA level only.  Sub-metro, ZIP-code, and neighbourhood
  variation is not captured.
- Scope: mid-tier SFR segment only.  Condo, rental, luxury, distressed, and
  commercial markets behave differently and are out of scope.
- No causal climate claims.  Weather and disaster features are
...
3. FRED MORTGAGE30US (national mortgage rate) and CPIAUCSL (CPI / inflation)
4. Confidence intervals via bootstrapped prediction or quantile regression
5. Condo / rental data if scope expands beyond SFR
